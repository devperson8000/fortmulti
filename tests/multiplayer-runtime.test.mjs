...
test('movement and inventory transitions are forwarded immediately',()=>{
 ...
 assert.equal(inputTransition(prev,{...prev,x:1}),true);assert.equal(inputTransition(prev,{...prev,slot:5}),true);
...
test('long snapshot gaps are treated as stale so reconnects do not extrapolate old motion',()=>{
 assert.equal(isStaleSnapshotGap(.2),false);
 assert.equal(isStaleSnapshotGap(.74),false);
 assert.equal(isStaleSnapshotGap(.76),true);
 assert.equal(isStaleSnapshotGap(3),true);
});
