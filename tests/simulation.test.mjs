...
import {AETHER_FLIGHT,SKYSHIP_TIMELINE,stepAetherFlight} from '../public/skyship-sequence.js';
...
test('four-player match starts aboard the Aether Ark and auto-launches into a smooth winged landing',()=>{
 ...
 assert.ok(m.players.every(p=>p.hp<=0||p.air==='landed'));assert.ok(m.events.some(e=>e.type==='rift_auto'));assert.ok(m.events.some(e=>e.type==='wings_open'&&e.forced));assert.equal(m.phase,'playing');
});
test('the skyship intro is higher than the terrain and a late exit uses the authored rift transition',()=>{
 ...
 for(let i=0;i<SKYSHIP_SECONDS/.05;i++)m.tick(.05);assert.equal(m.players[0].air,'riftTransit');assert.equal(m.players[0].launchProgress,0);assert.ok(m.events.some(e=>e.type==='rift_auto'&&e.by==='a'));
});
test('energy wings open automatically at the clearance boundary and keep descent controllable',()=>{
 const m=new Match(world,['a','b']);m.phase='flight';const p=m.players[0];p.air='rift';p.p=[0,45,0];p.airVelocity=[0,-8,0];
 for(let i=0;i<30&&p.air==='rift';i++)m.tick(.05);assert.equal(p.air,'wingOpening');assert.ok(p.deploy>0&&p.deploy<1);assert.ok(m.events.some(e=>e.type==='wings_open'&&e.forced));
 for(let i=0;i<30&&p.air!=='winged';i++)m.tick(.05);assert.equal(p.air,'winged');assert.ok(p.airVelocity[1]<0&&p.airVelocity[1]>-7);
});
test('aether flight speed and vertical motion stay bounded',()=>{
 assert.ok(AETHER_FLIGHT.wingFall>5&&AETHER_FLIGHT.wingFall<7);
 let velocity=[0,0,0];for(let i=0;i<120;i++)velocity=stepAetherFlight(velocity,{yaw:.2,pitch:.1,x:.4,z:1,sprint:true},true,.05);assert.ok(velocity.every(Number.isFinite));assert.ok(Math.hypot(...velocity)<=AETHER_FLIGHT.maxSpeed);assert.ok(velocity[1]<0);
});
...
test('rift movement uses eased vectors and landing does not alter ground movement physics',()=>{
 const m=new Match(world,['a','b']);m.phase='flight';const p=m.players[0];p.air='rift';p.p=[0,120,0];p.airVelocity=[0,0,0];
 m.input('a',{yaw:0,pitch:.2,z:1});m.tick(.05);const first=p.airVelocity.slice();assert.ok(first.every(Number.isFinite));assert.ok(first[2]<0);assert.ok(p.airSpeed<=AETHER_FLIGHT.maxSpeed);
 for(let i=0;i<30;i++){m.input('a',{yaw:.8,pitch:-.2,x:1,z:0});m.tick(.05);}assert.ok(p.airVelocity.every(Number.isFinite));assert.ok(p.airSpeed<=AETHER_FLIGHT.maxSpeed);
});
test('terrain clearance automatically unfolds the energy wings and locks folding near the ground',()=>{
 const elevated={height:()=>0,obstacles:[{min:[-5,0,-5],max:[5,20,5]}]},m=new Match(elevated,['a','b']);m.phase='flight';const p=m.players[0];p.air='rift';p.p=[0,48,0];p.airVelocity=[0,-8,0];m.tick(.05);
 assert.equal(p.air,'wingOpening');assert.equal(p.wingsActive,true);assert.ok(m.events.some(e=>e.type==='wings_open'&&e.by==='a'&&e.forced));
 for(let i=0;i<20;i++)m.tick(.05);assert.equal(p.air,'winged');m.input('a',{jump:true,yaw:0});m.tick(.05);assert.equal(p.air,'winged');assert.ok(m.events.some(e=>e.type==='wings_fold_blocked'&&e.by==='a'));
});
test('winged flight can fold into the aether current above the safety band',()=>{
 const m=new Match(world,['a','b']);m.phase='flight';const p=m.players[0];p.air='winged';p.wingsActive=true;p.deploy=1;p.p=[0,120,0];p.airVelocity=[0,-5.6,0];
 m.input('a',{jump:true,yaw:0});m.tick(.05);assert.equal(p.air,'wingFolding');m.input('a',{jump:false,yaw:0});for(let i=0;i<12;i++)m.tick(.05);assert.equal(p.air,'rift');assert.equal(p.wingsActive,false);assert.ok(m.events.some(e=>e.type==='rift_resume'&&e.by==='a'));
});
test('a player can move and shoot as soon as they land while another player is airborne',()=>{
 const m=new Match(world,['a','b']);m.beginSkyshipJourney();m.phase='flight';const [landed,airborne]=m.players;landed.air='landed';landed.p=[0,0,0];landed.vy=0;airborne.air='rift';airborne.p=[0,120,-11];airborne.vy=-4;airborne.airVelocity=[0,-4,0];airborne.airPitch=0;m.input('a',{yaw:0,pitch:.13,z:1,fire:true});m.tick(.1);assert.equal(m.phase,'playing');assert.equal(landed.air,'landed');assert.ok(landed.p[2]<-.3,'grounded player should move immediately');assert.ok(landed.weapons.ar.ammo<30,'grounded player should be able to shoot immediately');assert.ok(m.events.some(e=>e.type==='shot'&&e.by==='a'));assert.equal(airborne.air,'rift');m.tick(.1);assert.ok(m.elapsed>=.09,'round time should continue while other players remain airborne');assert.equal(airborne.air,'rift');
});

test('mouse look works inside the Ark while fire cannot trigger the rift exit',()=>{
 const m=new Match(world,['a','b']);m.beginSkyshipJourney();const before=m.players[0].yaw;m.input('a',{yaw:1.1,fire:true});m.tick(.05);assert.equal(m.players[0].air,'ship');assert.notEqual(m.players[0].yaw,before);assert.equal(m.events.some(e=>e.type==='rift_launch'),false);
});
test('skyship snapshots contain finite lightweight sequence and flight state',()=>{
 ...
 assert.equal(state.sequence.stage,'ready');assert.ok(state.skyship&&Number.isFinite(state.skyship.y));assert.ok(['ship','riftTransit'].includes(p.air));assert.ok(p.airVelocity.length===3&&p.airVelocity.every(Number.isFinite));
