import {mkdir,cp,writeFile,rm,access} from 'node:fs/promises';
await rm('dist',{recursive:true,force:true});await mkdir('dist',{recursive:true});await cp('public','dist',{recursive:true});
for(const file of ['app.js','engine.js','network.js','simulation.js','connection-stability.js','multiplayer-runtime.js','index.html'])await access('dist/'+file);
...
await writeFile('dist/config.js','window.SUNNY_CONFIG='+JSON.stringify({url,key})+';\nPromise.all([import("./connection-stability.js"),import("./multiplayer-runtime.js")]).catch(error=>console.error("Multiplayer stability runtime failed to load",error));\n');
