...
const BUS_START=[-155,86],BUS_END=[155,-86],BUS_ALTITUDE=76,BUS_SECONDS=12;
...
export class Match{
 ...
 busAt(t){const q=clamp(t/BUS_SECONDS,0,1),x=BUS_START[0]+(BUS_END[0]-BUS_START[0])*q,z=BUS_START[1]+(BUS_END[1]-BUS_START[1])*q,y=BUS_ALTITUDE+Math.sin(q*Math.PI)*3,yaw=Math.atan2(-(BUS_END[0]-BUS_START[0]),-(BUS_END[1]-BUS_START[1]));return {x,y,z,yaw,progress:q,active:q<1};}
 startRound(waiting=false){
  ...
  this.pickups=this.mode==='town'?[{x:0,z:8,type:'shield'},{x:-22,z:20,type:'wood'},{x:25,z:8,type:'health'},{x:36,z:-18,type:'wood'},{x:-38,z:-9,type:'shield'},{x:8,z:38,type:'health'}]:[];
  this.bus=this.busAt(0);
  this.players=this.ids.map((id,i)=>{const off=seatOffset(i);return {id,p:[this.bus.x+off[0],this.bus.y+.1,this.bus.z+off[2]],yaw:this.bus.yaw,vy:0,hp:100,shield:100,ammo:30,material:150,reload:0,cool:0,walk:0,input:sanitize(),lastInput:0,air:'bus',jumpLatch:false,eliminated:false};});
  this.events=[];
 }
 ...
 updateDrop(dt){
  ...
  for(let index=0;index<this.players.length;index++){
   ...
   }else if(p.air==='freefall'||p.air==='glider'){
    anyDropped=true;p.yaw=i.yaw;const steer=p.air==='glider'?10:13,len=Math.max(1,Math.hypot(i.x,i.z)),dx=(Math.cos(i.yaw)*i.x-Math.sin(i.yaw)*i.z)/len*steer*dt,dz=(-Math.sin(i.yaw)*i.x-Math.cos(i.yaw)*i.z)/len*steer*dt;p.p[0]=clamp(p.p[0]+dx,-160,160);p.p[2]=clamp(p.p[2]+dz,-160,160);
    const floor=ground(p.p[0],p.p[2],p.p[1],this.structures,this.world),clearance=p.p[1]-floor;
    if(p.air==='freefall'&&(edgeJump||clearance<=27)){p.air='glider';this.event({type:'deploy',by:p.id});}
    p.vy=p.air==='glider'?-6.2:(i.sprint?-24:-18);p.p[1]+=p.vy*dt;
