...
export const ARCADE_FLIGHT=Object.freeze({launchSpeed:8.2,gliderSpeed:10.8,launchFall:8,gliderFall:5.6,launchTurnAngle:.28,gliderTurnAngle:.48,boostMultiplier:1.12,maxRise:.35,autoOpenClearance:39,foldBuffer:18,openSeconds:.8,foldSeconds:.46,maxSpeed:13.5});
...
export function stepArcadeGlide(velocity,input={},glider=0,dt=1/60){
 ...
 const x=clamp(Number.isFinite(input.x)?input.x:0,-1,1),z=clamp(Number.isFinite(input.z)?input.z:0,-1,1);
 const gliderBlend=typeof glider==='number'?clamp(glider,0,1):glider?1:0,ease=gliderBlend*gliderBlend*(3-2*gliderBlend),turnAngle=ARCADE_FLIGHT.launchTurnAngle+(ARCADE_FLIGHT.gliderTurnAngle-ARCADE_FLIGHT.launchTurnAngle)*ease,heading=yaw-x*turnAngle,forward=z<-.01?-.68:1;
 const dirX=-Math.sin(heading)*forward,dirZ=-Math.cos(heading)*forward,speed=Math.min(ARCADE_FLIGHT.maxSpeed,(ARCADE_FLIGHT.launchSpeed+(ARCADE_FLIGHT.gliderSpeed-ARCADE_FLIGHT.launchSpeed)*ease)*(input.sprint?ARCADE_FLIGHT.boostMultiplier:1));
 const fall=ARCADE_FLIGHT.launchFall+(ARCADE_FLIGHT.gliderFall-ARCADE_FLIGHT.launchFall)*ease,pitchResponse=1.8+(8.4-1.8)*ease,target=[dirX*speed,clamp(-fall+pitch*pitchResponse,-9.5,ARCADE_FLIGHT.maxRise),dirZ*speed],rate=3.5+(4.5-3.5)*ease,blend=1-Math.exp(-rate*step);
 const current=Array.isArray(velocity)&&velocity.length===3?velocity:[0,0,0];
 return current.map((value,index)=>clamp(value+(target[index]-value)*blend,index===1?-9.5:-ARCADE_FLIGHT.maxSpeed,index===1?ARCADE_FLIGHT.maxRise:ARCADE_FLIGHT.maxSpeed));
}
