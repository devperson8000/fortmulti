...
function arcadeLaunchStreaks(eye,yaw,progress=0){
 const p=v=>cabinPoint(v,eye,yaw),amount=clamp(progress,0,1),near=1.1+amount*.7;
 for(let i=0;i<20;i++){const a=i/20*Math.PI*2,b=((i*7)%19)/19,side=Math.cos(a),up=Math.sin(a),start=[side*(.4+b*.7),up*(.3+b*.55),-near-b*1.2],end=[side*(1.2+b*2.5),up*(1+b*1.8),start[2]-2.2];beam(p(start),p(end),.014+(i%3)*.004,i%4?C.cream:C.blue);}
}
function firstPersonGlider(eye,yaw,amount=1,bank=0){
 const p=v=>cabinPoint(v,eye,yaw),open=clamp(amount,0,1),ease=open*open*(3-2*open);if(ease<.015)return;
 for(const side of[-1,1]){const root=[side*.24,-.4,-1.05],tip=[side*(.55+1.25*ease),.52+.8*ease,-2.55],outer=[side*(.72+1.55*ease),-.3+.3*ease,-2.1],low=[side*.4,-.52,-1.35];tri(p(root),p(tip),p(outer),color('66b5be'));tri(p(root),p(outer),p(low),color('3e798b'));beam(p(root),p(tip),.035,C.cream);beam(p(tip),p(outer),.028,C.blue);for(let k=1;k<4;k++){const t=k/4,a=root.map((v,i)=>v+(tip[i]-v)*t),b=root.map((v,i)=>v+(outer[i]-v)*t);beam(p(a),p(b),.012,C.cream);}}
}
...
function firstPersonGlider(eye,yaw,amount=1,bank=0){
 const p=v=>cabinPoint(v,eye,yaw),open=clamp(amount,0,1),ease=open*open*(3-2*open);if(ease<.015)return;
 for(const side of[-1,1]){const root=[side*.24,-.4,-1.05],tip=[side*(.55+1.25*ease),.52+.8*ease,-2.55],outer=[side*(.72+1.55*ease),-.3+.3*ease,-2.1],low=[side*.4,-.52,-1.35];tri(p(root),p(tip),p(outer),color('76dce7'));tri(p(root),p(outer),p(low),color('438cbe'));beam(p(root),p(tip),.035,C.cream);beam(p(tip),p(outer),.028,C.blue);for(let k=1;k<4;k++){const t=k/4,a=root.map((v,i)=>v+(tip[i]-v)*t),b=root.map((v,i)=>v+(outer[i]-v)*t);beam(p(a),p(b),.012,C.cream);}}
}
