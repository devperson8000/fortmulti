...
export class Voice{
 constructor(send,report){this.send=send;this.report=report;this.pending=new Map();this.pcs=new Map();this.audios=new Map();this.localId='';}
 async enable(localId,peerIds=[]){try{this.localId=localId;this.stream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true},video:false});this.sync(localId,peerIds);this.report(peerIds.length?'Connecting party voice…':'Microphone ready · waiting for party');}catch(e){this.stop();throw e;}}
 ...
 ensure(id){if(!id||id===this.localId||this.pcs.has(id)||!this.stream)return this.pcs.get(id);const extra=window.SUNNY_CONFIG?.iceServers||[],pc=new RTCPeerConnection({iceServers:[{urls:'stun:stun.l.google.com:19302'},...extra]});this.pcs.set(id,pc);this.pending.set(id,[]);this.stream.getTracks().forEach(t=>pc.addTrack(t,this.stream));pc.onicecandidate=e=>{if(e.candidate)this.send(id,{candidate:e.candidate});};pc.ontrack=e=>{let audio=this.audios.get(id);if(!audio){audio=new Audio();audio.autoplay=true;this.audios.set(id,audio);}audio.srcObject=e.streams[0];audio.play().catch(()=>this.report('Click Voice again if audio playback is blocked'));};pc.onconnectionstatechange=()=>{const states=[...this.pcs.values()].map(v=>v.connectionState),connected=states.filter(s=>s==='connected').length;if(connected)this.report(`Party voice · ${connected} connected`);else if(states.some(s=>s==='failed'))this.report('Some party voice links are blocked by the network');};if(this.localId<id)queueMicrotask(()=>this.offer(id).catch(()=>{}));return pc;}
 ...
 async receive(from,m){if(!this.stream)return;const pc=this.ensure(from);if(!pc)return;if(m.description){if(m.description.type==='offer'&&pc.signalingState!=='stable'){try{await pc.setLocalDescription({type:'rollback'});}catch{}}await pc.setRemoteDescription(m.description);for(const c of this.pending.get(from)?.splice(0)||[])await pc.addIceCandidate(c);if(m.description.type==='offer'){await pc.setLocalDescription(await pc.createAnswer());this.send(from,{description:pc.localDescription});}}else if(m.candidate){if(pc.remoteDescription)await pc.addIceCandidate(m.candidate);else this.pending.get(from)?.push(m.candidate);}}
 remove(id){const pc=this.pcs.get(id);pc?.close();this.pcs.delete(id);this.pending.delete(id);const audio=this.audios.get(id);if(audio)audio.srcObject=null;this.audios.delete(id);}
 mute(){if(!this.stream)return false;const tracks=this.stream.getAudioTracks(),enabled=!tracks[0]?.enabled;tracks.forEach(t=>t.enabled=enabled);return enabled;}
 stop(){this.stream?.getTracks().forEach(t=>t.stop());for(const pc of this.pcs.values())pc.close();for(const audio of this.audios.values())audio.srcObject=null;this.pcs.clear();this.pending.clear();this.audios.clear();this.stream=null;}
}
