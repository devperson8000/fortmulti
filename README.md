...
Sunny Skirmish is a static, browser-based multiplayer prototype built for Vercel. Every playable sequence stays first person: a cinematic Aether Ark cabin intro, an open-rift launch, and smooth arcade energy-wing flight before the island match. Private parties support **2–8 players** with an authoritative leader simulation, online presence and party invites, ready-up, shared building, text chat, optional party voice, reconnect handling and last-player-standing rounds.
...
Lobby characters use a dedicated relaxed idle pose with their arms naturally lowered. The character rig has separate idle, combat, Aether-flight, equip, aim and reload targets.
...
The v6.2 presentation pass replaces the remaining block-jointed silhouette with a higher-density capsule-and-ovoid character. Rounded shoulders, elbows, hands, hips, knees, ankles, boots, layered hair and curved outfit panels overlap cleanly during the relaxed lobby pose, combat and Aether-flight poses. Party members now share consistent natural skin materials instead of receiving the old opponent tint. The lobby chrome uses rounded layered cards, softer controls, a curved slide-out social drawer and a compact voice panel while keeping the local character unobstructed at the front.
...
- The host remains authoritative for movement, building, damage, ammo, storm damage, eliminations, scoring and the Aether Ark sequence.
- Late joiners are rejected while a match is in progress and can be invited again from the lobby.

## Island and Aether Ark sequence

Every round opens in first person inside the **Aether Ark**, an original floating sky-vessel built from carved timber, brass trim, a faceted levitation crystal and crescent-shaped sails. Its slow route crosses the island for roughly 32 seconds at a 240-unit altitude, with gentle banking, floating vapor and a luminous front rift.

The opening is staged instead of handing over control immediately:

1. The player begins seated while cabin lanterns and the Ark rise through the clouds.
2. The view lifts smoothly as the player stands; the front rift opens and the Ark’s doors slide aside.
3. Mouse look remains active. Once control unlocks, the player can walk down the aisle and enter the open rift with **Space**.
4. Waiting too long starts a short guided rift transition automatically, then returns control in the Aether current.
5. The current has bounded, eased steering. **Space** unfurls luminous energy wings; they open over time and automatically near the terrain boundary. At safe height, Space folds them back into the current. The wings steer, bank, and descend at an arcade pace before landing.

The first player can move, build and fight as soon as they land; other players keep their live aerial sequence until they touch down. Each player’s cabin stage, portal transition, wing state, position and velocity are synchronized through the existing host snapshots.

The first-person cabin is built from view-relative wall ribs, benches, deck panels, lanterns, guidance consoles and animated portal doors so it stays readable on different display sizes. Outside, the Aether Ark uses a sculpted hull, crystal sail, crescent wings, rune lighting and drifting cloud wisps. In flight, a rotating rift tunnel and first-person wing facets keep the motion legible without switching out of first person.
...
| Control | Action |
| --- | --- |
| WASD | Walk the Ark aisle; steer the Aether current |
| Mouse | Look around in first person |
| Left mouse | Fire / place selected build |
| Right mouse | ADS / sniper scope |
| Space | Enter the rift / open or fold energy wings |
| Shift | Sprint / brief Aether surge |
| 1 | Striker AR |
| 2 | Thunder Shotgun |
| 3 | Burst SMG |
| 4 | Eagle-Eye Sniper |
...
The lobby remains third person. The match intro, Aether flight and combat stay first person.
